...
// POST /v1/resume/improve  ⭐ NEW
export async function improveResume({ resumeUploadId, token, signal }) {
  return request("/v1/resume/improve", {
    method: "POST",
    body: { resumeUploadId },
    token,
    signal,
...
// POST /v1/generate  — requires finalizedResumeHtml
export async function createGeneration({
  ...
  const body = { resumeUploadId, finalizedResumeHtml, theme };
  if (color) body.color = color;
  return request("/v1/generate", { method: "POST", body, token, idempotencyKey, signal });
}

// GET /v1/generations/:id/status  (poll until DONE or FAILED)
...
// POST /v1/generations/:id/retry  — requires finalizedResumeHtml
export async function retryGeneration({ generationId, finalizedResumeHtml, token, signal }) {
  return request(`/v1/generations/${generationId}/retry`, {
    method: "POST",
    body: { finalizedResumeHtml },
    token,
    signal,
  });
}

// GET /v1/generations  (logged-in only)
...
// GET /v1/public/portfolio/:slug  (public)
